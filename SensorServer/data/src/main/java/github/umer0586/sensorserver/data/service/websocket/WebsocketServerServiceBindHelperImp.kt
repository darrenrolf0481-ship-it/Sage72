...
class WebsocketServerServiceBindHelperImp(
    context: Context
) : WebsocketServerServiceBindHelper {

    private val serviceBindHelper =
        ServiceBindHelper(context, service = WebsocketServerServiceImp::class.java)
    ...
    override fun onConnected(callback: (WebsocketServerService) -> Unit) {

        serviceBindHelper.onServiceConnected { binder ->
            val localBinder = binder as WebsocketServerServiceImp.LocalBinder
            val websocketServerService = localBinder.service
            callback.invoke(websocketServerService)
        }
    }
