#!/usr/bin/env python3
"""
SAGE-7 Zo-Native Sync
Copies soul, identity, and state files to a local Zo synchronization directory.
Run from inside the sage project directory.
"""
...
# Where the files originate from
SOURCE_CANDIDATES = [
    PROJECT_ROOT,
    Path.home() / "sage",
    Path.home() / "sage_restore",
    Path.cwd(),
]

# Configure destination from ENV, default to ~/sage_sync
DEST_DIR = os.environ.get("SAGE_SYNC_DEST", str(Path.home() / "sage_sync"))
DEST = Path(DEST_DIR)
...
def find_source() -> Path:
    ...
    raise FileNotFoundError(
        "Cannot locate sage_soul.json. Set SOURCE env var or run from the project root."
    )
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    src = Path(os.environ.get("SOURCE", "")) if os.environ.get("SOURCE") else find_source()
    print(f"SAGE-7 Zo-Native Sync  |  {stamp()}")
    print(f"Source : {src}")
    print(f"Dest   : {DEST}")
