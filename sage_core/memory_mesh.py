...
def recall_associative_pathways(query: str, limit: int = 6, depth: int = 2) -> List[Dict[str, Any]]:
    ...
    Optimized: Pre-computes lowercased graph node names once per query and uses O(1) set lookups.
    """
    ...
    try:
        ...
        # Cache lowercased graph node names once to prevent repeated .lower() allocations across keywords
        ...
        for kw in keywords:
            for node, node_lower in graph_nodes:
                ...
                if kw in node_lower or node_lower in kw:
                    ...
                    extended = []
                    if depth >= 2:
                        # O(1) set lookup instead of O(N) list scanning
